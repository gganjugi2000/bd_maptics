...
(
    ...
    `COMP_IMG`     VARCHAR(255)    NULL        COMMENT '업체 이미지', 
...
(
    `FILE_SEQ`      INT         NOT NULL    AUTO_INCREMENT COMMENT '파일속성일련번호', 
...
ALTER TABLE COM_FILE
    ADD CONSTRAINT FK_COM_FILE_ATCH_FILE_ID_USER_INFO_ATCH_FILE_ID FOREIGN KEY (ATCH_FILE_ID)
        REFERENCES USER_INFO (ATCH_FILE_ID) ON DELETE RESTRICT ON UPDATE RESTRICT;


CREATE TABLE COM_FILE_DETAIL
(
    `FILE_ATTR_SEQ`   INT             NOT NULL    AUTO_INCREMENT COMMENT '파일상세일련번호', 
    `ATCH_FILE_ID`    CHAR(20)        NOT NULL    COMMENT '첨부파일ID', 
    ...
    `UPD_ID`          VARCHAR(20)     NULL        COMMENT '수정ID', 
    `UPD_DT`          DATETIME        NULL        COMMENT '수정일시', 
    PRIMARY KEY (FILE_ATTR_SEQ, ATCH_FILE_ID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

ALTER TABLE COM_FILE_DETAIL COMMENT '파일 속성 상세';
